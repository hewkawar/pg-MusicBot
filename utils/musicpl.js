...
// Downloader
async function dlSpotify(interaction, dlPath, getResults, emoji_name, emoji_id, spotify_client_id, spotify_client_secret, config) {
    ...
    const updateProgressBar = async () => {
        await dlMessage.edit({
            embeds: [new EmbedBuilder().setColor(config.Color).setTitle(`<:${emoji_name}:${emoji_id}> ${lang(interaction.locale, "commands.play.excute.download")}`).setDescription(progressBar[currentProgress])]
        });
        currentProgress++;
    };

    const dlMessage = await interaction.channel.send({
        embeds: [new EmbedBuilder().setColor(config.Color).setTitle(`<:${emoji_name}:${emoji_id}> ${lang(interaction.locale, "commands.play.excute.download")}`).setDescription(progressBar[0])]
    });
...
async function dlYoutube(interaction, dlPath, getResults, emoji_name, emoji_id, config) {
    if (fs.existsSync(dlPath)) {
        return;
    }
...
async function playMusic(interaction, dlPath, connection, player, platform, platform_emoji_id, song_name, config) {
    let resource = createAudioResource(createReadStream(dlPath));

    player.play(resource);

    await interaction.channel.send({ contant: '@silent', embeds: [new EmbedBuilder().setColor(config.Color).setDescription(`<:${platform}:${platform_emoji_id}> ${lang(interaction.locale, "commands.play.excute.start_playing")} **${song_name}**`)] });
...
async function playSpotify(interaction, searchResults, ac_token, config, spotify_client_id, spotify_client_secret) {
    if (!searchResults) {
    ...
    const cardEmbed = new EmbedBuilder()
        ...
        .addFields(
            { name: `${lang(interaction.locale, "commands.play.excute.album")}`, value: `\`${getResults.album}\``, inline: true },
            { name: lang(interaction.locale, "commands.play.excute.voice_chat"), value: `<#${interaction.member.voice.channel.id}>`, inline: true },
            { name: lang(interaction.locale, "commands.play.excute.owner"), value: `<@${interaction.user.id}>`, inline: true }
        );
    ...
    await dlSpotify(interaction, dlPath, getResults, 'spotify', '1185497734678061096', spotify_client_id, spotify_client_secret, config);

    await playMusic(interaction, dlPath, connection, player, 'spotify', '1185497734678061096', getResults.name, config);
}

async function playYoutube(interaction, searchResults, config, onriginalMessage) {
    if (!searchResults) {
    ...
    const cardEmbed = new EmbedBuilder()
        ...
        .setThumbnail(getResults.images)
        .addFields(
            { name: lang(interaction.locale, "commands.play.excute.view"), value: `\`${getResults.view}\``, inline: true },
            { name: lang(interaction.locale, "commands.play.excute.voice_chat"), value: `<#${interaction.member.voice.channel.id}>`, inline: true },
            { name: lang(interaction.locale, "commands.play.excute.owner"), value: `<@${interaction.user.id}>`, inline: true }
        );
    ...
    await dlYoutube(interaction, dlPath, getResults, 'youtube', '1185497741460250685', config);

    await playMusic(interaction, dlPath, connection, player, 'youtube', '1185497741460250685', getResults.name, config);
}
